  ...
  document.querySelectorAll(".adet-controller").forEach((ekleCikarAlani) => {
    ...
    minus.onclick = () => {
      ...
      if (adet.textContent < 1) {
        alert(
          "Üründen vaz mı geceçeksiniz. Bir daha mı düşünsenizyanında tarak bedava"
        );
...
h1.onclick = () => {
  ...
  flag
    ...
    : (h1.textContent = "almazsan ölürsün");
};
...
header.onclick = () => {
  ...
  flag
    ? (h1.textContent = "Arkaplan daha baskındır")
    : (h1.textContent = "Babana yol ver");
};
